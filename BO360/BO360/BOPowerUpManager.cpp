...
void BOPowerUpManager::AddPowerUp(PowerUpTypes p_type, float2 p_startPosition, int2 p_windowSize, BOPaddle* p_paddle, float2 p_blackholePosition)
{
	...
	switch (p_type)
	{
		case PUBiggerPad:
		{
			...
			biggerPad->Initialize(PUBiggerPad, p_startPosition, int2(30, 30), "Bilder/placeholderPowerup3.png", 500.0f, p_windowSize, p_paddle, p_blackholePosition);
		...
		case PUShield:
		{
			...
			shield->Initialize(PUShield, p_startPosition, int2(30, 30), "Bilder/placeholderPowerup2.png", 500.0f, p_windowSize, p_paddle, p_blackholePosition);
		...
		case PUExtraBall:
		{
			...
			extraBall->Initialize(PUExtraBall, p_startPosition, int2(40, 40), "Bilder/placeholderPowerupMultBall.png", 500.0f, p_windowSize, p_paddle, p_blackholePosition);
